...
T(5,:)=mean(train4); T(6,:)=mean(train5); T(7,:)=mean(train6); T(8,:)=mean(train7); T(9,:)=mean(train8); T(10,:)=mean(train9);
...
%our testDigit function accurately determines the digit d.
for j=1:12
    d=double(test1(j,:));
    testDigit(d,T)
end
